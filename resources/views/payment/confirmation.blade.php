...
@section('title', 'Konfirmasi Pembayaran - Car Wash Booking')
                        ...
                        <div class="col-md-6">
                            ...
                            <table class="table table-borderless">
                                <tr>
                                    ...
                                    <td>Rp {{ number_format($order->original_price ?? 0, 0, ',', '.') }}</td>
                                </tr>
                                @if(($order->discount_percent ?? 0) > 0)
                                <tr>
                                    <td><strong>Diskon ({{ $order->discount_percent }}%):</strong></td>
                                    <td class="text-success">- Rp {{ number_format(($order->original_price ?? 0) * ($order->discount_percent ?? 0) / 100, 0, ',', '.') }}</td>
                                </tr>
                                @if($order->discount_name)
                                <tr>
                                    <td><strong>Nama Diskon:</strong></td>
                                    <td><span class="badge bg-success">{{ $order->discount_name }}</span></td>
                                </tr>
                                @endif
                                @endif
                                <tr class="border-top">
                                    <td><strong>Total Pembayaran:</strong></td>
                                    <td class="h5 text-primary"><strong>Rp {{ number_format($order->final_price ?? $order->original_price ?? 0, 0, ',', '.') }}</strong></td>
                                </tr>
                            </table>
                        </div>
                    </div>
