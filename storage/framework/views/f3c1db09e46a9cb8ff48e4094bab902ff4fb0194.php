                    ...
                    <form action="<?php echo e(route('booking.store')); ?>" method="POST" id="booking-form">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="booking_date" name="booking_date" required 
                                   min="<?php echo e(date('Y-m-d')); ?>" max="<?php echo e(date('Y-m-d', strtotime('+14 days'))); ?>">
                        ...
                        <div class="mb-3" id="time-slots-section" style="display: none;">
                            ...
                            <input type="hidden" id="selected_slot_time" name="selected_slot_time">
                        </div>
    ...
    async function loadTimeSlots(date) {
        ...
        try {
            ...
            if (data.status === 'success' && data.data.length > 0) {
                ...
                timeSlots.innerHTML = data.data.map(slot => {
                    ...
                    return `
                        <div class="col-md-4 col-lg-3 mb-2">
                            <div class="card time-slot ${isAvailable ? 'border-success' : 'border-secondary'} ${!isAvailable ? 'opacity-50' : ''}"
                                 onclick="${isAvailable ? `selectSlot('${slot.jam_mulai}', '${slot.jam_selesai}', ${slot.tersedia})` : ''}"
                                 style="cursor: ${isAvailable ? 'pointer' : 'not-allowed'}">
                                <div class="card-body text-center p-2">
                                    <div class="fw-bold">${slot.jam_mulai} - ${slot.jam_selesai}</div>
                                    <small class="text-muted">
                                        ${isAvailable ? `${slot.tersedia} slot tersisa` : 'Penuh'}
                                    </small>
                                </div>
                            </div>
                        </div>
                ...
                timeSlotsSection.style.display = 'block';
                timeSlots.innerHTML = '<div class="col-12"><div class="alert alert-warning">Tidak ada slot tersedia untuk tanggal ini</div></div>';
            }
        } catch (error) {
            console.error('Failed to load time slots:', error);
            timeSlotsSection.style.display = 'block';
            timeSlots.innerHTML = '<div class="col-12"><div class="alert alert-danger">Gagal memuat slot waktu</div></div>';
        }
    }
    
    window.selectSlot = function(jamMulai, jamSelesai, tersedia) {
        // Remove previous selection
        timeSlots.querySelectorAll('.time-slot').forEach(el => {
        ...
        // Set selected time
        const selectedTime = `${jamMulai} - ${jamSelesai}`;
        selectedSlotTime.value = selectedTime;
        ...
        bookingDetails.innerHTML = `
            ...
            <div><strong>Waktu:</strong> ${selectedTime}</div>
...
<?php echo $__env->make('layouts.carwash', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\CarWashConnect\resources\views/booking_form.blade.php ENDPATH**/ ?>